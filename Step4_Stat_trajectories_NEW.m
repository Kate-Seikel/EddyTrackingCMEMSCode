...
close all
clc
input_dir= [strcat(basepath, 'EXTRACTION/EDDY_PROPERTIES')]; %EDDY_PROPERTIES'];
...
mean_delt_Ampl = mean(delt_Ampl(:),'omitnan');
std_delt_Ampl = std(delt_Ampl(:),'omitnan');

mean_delt_Rad = mean(delt_Rad(:),'omitnan');
std_delt_Rad = std(delt_Rad(:),'omitnan');

mean_delt_EKE = mean(delt_EKE(:),'omitnan');
std_delt_EKE = std(delt_EKE(:),'omitnan');

mean_delt_dist = mean(delt_dist_center(:),'omitnan');
std_delt_dist = std(delt_dist_center(:),'omitnan');
...
eval([' save -append ' filename '  mean* std* numtraj'])
