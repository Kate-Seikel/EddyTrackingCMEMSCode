...
%% load parameters
function Step5_Trajectories(basepath,fullextract)
...
for j=1:nc-1 % for all timesteps
    ...
    for i=idx % Loop on each eddy (from the biggest to the smallest)
        if  numtraj == n_traj       % Save matrix every n_traj trajectories
            ...
            save([output_dir, '/AE_Trajectories', aa_2, '.mat'],'AEs_traj','idtraj','split_id','numtraj','lifetime') %Create new matrix
...
aa = aa+1;
if aa<10; aa_2=['0',num2str(aa)]; else aa_2=num2str(aa); end
save([output_dir, '/AE_Trajectories', aa_2, '.mat'],'AEs_traj','idtraj','split_id','numtraj','lifetime','ID_anti') %Create new matrix
disp(['AEs ' aa_2 ' successfully saved'])
...
for j=1:nc-1 % for all timesteps
    ...
    for i=idx % Loop on each eddy (from the biggest to the smallest)
        if  numtraj == n_traj       % Save matrix every n_traj trajectories
            aa = aa+1;
            if aa<10; aa_2=['0',num2str(aa)]; else aa_2=num2str(aa); end
            save([output_dir, '/CE_Trajectories', aa_2, '.mat'],'CEs_traj','idtraj','split_id','numtraj','lifetime') %Create new matrix
...
aa = aa+1;
if aa<10; aa_2=['0',num2str(aa)]; else aa_2=num2str(aa); end
save([output_dir, '/CE_Trajectories', aa_2, '.mat'],'CEs_traj','idtraj','split_id','numtraj','lifetime','ID_cyclo') %Create new matrix
disp(['CEs ' aa_2 ' successfully saved'])
