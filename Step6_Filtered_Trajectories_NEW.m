...
%% AEs
function Step6_Filtered_Trajectories(minlife, minamp, minrad, basepath)
...
input_dir = [strcat(basepath,'EXTRACTION/EDDY_TRAJECTORIES/')]; %EDDY_TRAJECTORIES/'];
output_dir = [strcat(basepath,'EXTRACTION/EDDY_TRAJECTORIES/')]; %EDDY_TRAJECTORIES/'];
...
L = NaN([1 max(idtraj(:))*length(list)]);
A = NaN([1 max(idtraj(:))*length(list)]);
R = NaN([1 max(idtraj(:))*length(list)]);
...
traj(max(idtraj(:))*length(list)).Lifetime = 0;
...
for i=1:length(list)
    ...
    A(a+1:a+numtraj)=arrayfun(@(x) median(double(x.Amplitude),'omitnan'),AEs_traj(1:numtraj));
    R(a+1:a+numtraj)=arrayfun(@(x) median(double(x.Radius),'omitnan'),AEs_traj(1:numtraj));
...
save([output_dir 'AE_Filtered_Trajectories.mat'],'AE_traj','Fields_traj','date_num')
...
for i=1:length(list)
    ...
    A(a+1:a+numtraj)=arrayfun(@(x) median(double(x.Amplitude),'omitnan'),CEs_traj(1:numtraj));
    R(a+1:a+numtraj)=arrayfun(@(x) median(double(x.Radius),'omitnan'),CEs_traj(1:numtraj));
...
save([output_dir 'CE_Filtered_Trajectories.mat'],'CE_traj','Fields_traj','date_num')
